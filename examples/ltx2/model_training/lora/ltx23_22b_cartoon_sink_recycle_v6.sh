# v6 (2026-06-13): FULL SWITCH to LTX-2.3-22b + its union IC-LoRA, plus the
# crop-then-resize dataset fix (v5 first-chunk deformation = squished training
# anatomy). Same dims as 19b (4096/48 layers) so GAN/LoRA configs carry over.
# 2.3 union is ref0.5 (trained at reference attention strength 0.5; DiffSynth
# has no attention mask — known asymmetry, accepted as the official example
# ships the same way).
# v5 (2026-06-13): v4 + swap Union-Control → DEDICATED Pose-Control IC-LoRA.
# v4 eval showed control ≈ no effect: the 19B series has NO official
# union-control — pose is a dedicated LoRA (LTX-2-19b-IC-LoRA-Pose-Control,
# see official README). Union (a 2.3-22b lineage thing) is weak/blind to
# skeletons.
...
accelerate launch \
  ...
  --output_path "./models/train/LTX-2.3-22b_lora_cartoon_sink_recycle_v6" \
  --lora_base_model "dit" \
