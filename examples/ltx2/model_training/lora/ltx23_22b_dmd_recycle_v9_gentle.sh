...
#
# v8 (2026-06-16): GRAD CLIP. v7 (lower lr+alpha) only DELAYED the blow-up
# (v6 ~step500-600, v7 ~540-620) — same err_norm runaway. lr/alpha tuning
# insufficient. v8 adds --grad_clip 0.5 (clip_grad_norm_ on student & critic
# before each step) — the standard fix for the explosive updates (dmd_g→+11).
# Keeps v7's lr=2e-6 / alpha=0.1 / ramp=400. Started FRESH (not resumed from
# the diverged v7). Companion: *_norecycle.sh (error_alpha=0) isolates whether
# the instability is the recycle feedback or DMD-on-22B itself.
#
# v7 (2026-06-16): DIVERGENCE FIX (insufficient). v6 blew up — err_norm grew monotonically
...
OUTPUT_DIR="./models/train/ltx23_22b_dmd_recycle_v9_gentle"
...
accelerate launch \
    ...
    --error_alpha 0.05 --error_buffer_size 500 \
    --grad_clip 0.5 \
    --error_warmup_count 50 --error_alpha_ramp_steps 800 \
