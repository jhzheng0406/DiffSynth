"""
DMD2 + ONE-FORCING (GAN) + VGG-FEATURE HIGH-FREQUENCY AUX LOSS  (v2).

v1 (pixel Laplacian) had ~0 effect — gradients were drowned by GAN's own
high-freq matching signal from cls_branch on critic features.
v2 puts HF in a *different feature space* (VGG early layers) so it's
orthogonal to GAN's view.

    feats_fake_l = VGG_layer_l(VAE_dec(x_pred))   for l in early layers
    feats_real_l = VGG_layer_l(target_video).detach()
    hf_loss = sum_l MSE(feats_fake_l, feats_real_l)
    gen_loss = dmd_g + gan_g_weight * gan_g + highfreq_weight * hf_loss

Why VGG EARLY layers (relu1_2, relu2_2)?
  * Full / half spatial resolution → small textures get encoded
  * Late layers (relu3_3, relu4_3) are semantic — same blind spot as
    DINOv2-based FD-loss which failed on clothing detail.
  * CNN inductive bias is different from GAN's transformer-feature cls_branch.

Memory: VAE decode + VGG forward wrapped in torch.utils.checkpoint.
Pixels resized to 224×224 (VGG-typical) to bound activation size.
"""
...
def encode_batch(pipe, batch, dtype, device, no_recent=False):
    ...
    return dict(
        ...
        target_video=target_video,                # NEW: pixels in [-1,1] for HF aux loss
...
# ===========================================================================
# VGG-feature high-frequency extractor — hooks early-layer activations.
# ===========================================================================
_IMAGENET_MEAN = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
_IMAGENET_STD  = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1)


class VGGHighFreq(torch.nn.Module):
    """Hook early layers of a pretrained VGG16. Frozen, eval mode.

    Output: list of feature maps in order [B*T, C_l, H_l, W_l] per layer.
    Layer indices into torchvision's `vgg16().features`:
        3 = relu1_2  (64ch, 1× input spatial)
        8 = relu2_2  (128ch, 1/2 spatial)
       15 = relu3_3  (256ch, 1/4 spatial)   — getting semantic, usually skip
    """
    def __init__(self, layer_indices=(3, 8), dtype=torch.bfloat16):
        super().__init__()
        import torchvision
        try:
        ...
        self.features = torch.nn.Sequential(*list(vgg)[: max_idx + 1])
        self.features.eval()
        for p in self.features.parameters():
            p.requires_grad_(False)
        self.features.to(dtype=dtype)
        self.dtype = dtype

    def forward(self, pixels: torch.Tensor):
        """pixels: [B*T, 3, H, W] in [-1, 1]."""
        x = pixels.to(self.dtype)
        x = (x + 1.0) * 0.5                                  # → [0, 1]
...
def vgg_hf_loss(vgg_hf: VGGHighFreq,
                fake_pixels: torch.Tensor,         # [B, 3, T, H, W] in [-1,1], grad
                real_pixels: torch.Tensor,         # same, no grad
                resize_to: int = 224) -> torch.Tensor:
    """Per-frame VGG-feature MSE summed across hooked layers."""
    B, C, T, H, W = fake_pixels.shape
    # → [B*T, 3, H, W]
    fk = fake_pixels.permute(0, 2, 1, 3, 4).reshape(B * T, C, H, W)
    rl = real_pixels.permute(0, 2, 1, 3, 4).reshape(B * T, C, H, W)
    if H != resize_to or W != resize_to:
        fk = F.interpolate(fk.float(), size=(resize_to, resize_to),
                           mode="bilinear", align_corners=False).to(fake_pixels.dtype)
        rl = F.interpolate(rl.float(), size=(resize_to, resize_to),
                           mode="bilinear", align_corners=False).to(real_pixels.dtype)
    feats_fk = vgg_hf(fk)
    with torch.no_grad():
        feats_rl = vgg_hf(rl)
    return sum(F.mse_loss(f, r) for f, r in zip(feats_fk, feats_rl))
...
def get_parser():
    ...
    # ─── VGG-feature high-freq aux loss (v2) ───
    p.add_argument("--highfreq_weight", type=float, default=0.0,
                   help="Weight on sum_l MSE(VGG_l(VAE_dec(x_pred)), "
                        "VGG_l(target_video)). 0 = off.")
    p.add_argument("--hf_vgg_layers", default="3,8",
                   help="VGG16.features indices to hook. Defaults to relu1_2 + relu2_2, "
                        "the texture-heavy early layers. 15 = relu3_3 (semantic).")
    p.add_argument("--hf_vgg_resize", type=int, default=224,
                   help="Resize pixels to this square before VGG forward.")
    p.add_argument("--hf_vae_chunk", type=int, default=4,
                   help="Decode latents in chunks of this many along time "
                        "to cap VAE activation peak during backward recompute. "
                        "0 = single decode call.")
...
def main():
    ...
    # ---------------- VGG-feature high-freq extractor (frozen) ----------------
    vgg_hf = None
    if args.highfreq_weight > 0.0:
        layer_idx = tuple(int(x) for x in args.hf_vgg_layers.split(","))
        vgg_hf = VGGHighFreq(layer_indices=layer_idx, dtype=dtype).to(device)
        if is_main:
            n_vgg = sum(p.numel() for p in vgg_hf.parameters())
            print(f"[setup] VGG-HF: {n_vgg/1e6:.1f}M params (frozen), "
                  f"layers={layer_idx}, resize={args.hf_vgg_resize}")
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            # ─── VGG-feature high-freq aux loss ─────────────────────────────
            # ENTIRE VAE+resize+VGG path is checkpointed → no VGG/VAE
            # activations live in VRAM after the forward call.  In backward
            # the whole chunk is recomputed (~2× compute for this op).
            if args.highfreq_weight > 0.0 and vgg_hf is not None:
                resize_to = args.hf_vgg_resize

                def _latent_to_vgg_feats(x_lat):
                    """x_lat [B,16,T_lat,h,w] → tuple(feature maps) at hooked layers.

                    Internally: VAE decode → permute → resize → VGG. All in one
                    checkpoint segment so peak VRAM = one chunk's worth.
                    """
                    px = student_pipe.vae.decode(
                        x_lat.to(dtype), device=device,
                    ).to(dtype).clamp(-1, 1)                            # [B,3,T,H,W]
                    B, C, T, H, W = px.shape
                    fk = px.permute(0, 2, 1, 3, 4).reshape(B * T, C, H, W)
                    if H != resize_to or W != resize_to:
                        fk = F.interpolate(
                            fk.float(), size=(resize_to, resize_to),
                            mode="bilinear", align_corners=False,
                        ).to(dtype)
                    return tuple(vgg_hf(fk))                            # tuple needed for checkpoint

                chunk = args.hf_vae_chunk
                if not (chunk and chunk > 0 and chunk < x_pred.shape[2]):
                    chunk = x_pred.shape[2]   # treat as one chunk

                feat_lists = None     # one list per hooked layer
                for ci in range(0, x_pred.shape[2], chunk):
                    sub = x_pred[:, :, ci:ci + chunk]
                    feats_chunk = torch.utils.checkpoint.checkpoint(
                        _latent_to_vgg_feats, sub, use_reentrant=False,
                    )
                    if feat_lists is None:
                        feat_lists = [[f] for f in feats_chunk]
                    else:
                        for fl, f in zip(feat_lists, feats_chunk):
                            fl.append(f)
                feats_fake = [torch.cat(fl, dim=0) for fl in feat_lists]

                # Real features — no grad needed, no checkpoint needed.
                with torch.no_grad():
                    rp = cond["target_video"].clamp(-1, 1)               # [B,3,49,H,W]
                    Br, Cr, Tr, Hr, Wr = rp.shape
                    rl = rp.permute(0, 2, 1, 3, 4).reshape(Br * Tr, Cr, Hr, Wr)
                    if Hr != resize_to or Wr != resize_to:
                        rl = F.interpolate(
                            rl.float(), size=(resize_to, resize_to),
                            mode="bilinear", align_corners=False,
                        ).to(dtype)
                    feats_real = vgg_hf(rl)

                # Align temporal dim if VAE produced != target frame count
                N_min = min(feats_fake[0].shape[0], feats_real[0].shape[0])
                hf_loss = sum(
                    F.mse_loss(f[:N_min], r[:N_min])
                    for f, r in zip(feats_fake, feats_real)
                )
            else:
                hf_loss = x_pred.new_zeros(())

            gen_loss = dmd_g_loss + g_loss + args.highfreq_weight * hf_loss
