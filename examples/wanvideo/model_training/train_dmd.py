"""
DMD2 distillation for Wan-Fun-V1.1-1.3B + sink LoRA.

Three models live in memory:
    teacher  = base + sink LoRA  (frozen)        → score_real
    student  = base + sink LoRA + student LoRA   (trainable, init from sink)
    critic   = base + sink LoRA + critic  LoRA   (trainable, init from sink)

Each training step:
    1. ChunkAwareDataset → (target_video, sink, recent, pose, prompt)
    2. encode all conditioning ONCE (T5, VAE, CLIP) using student's pipe
    3. critic update × dfake_gen_update_ratio (default 5)
    4. generator update × 1

This file does NOT use the regular DiffSynth runner — DMD has 2 optimizers
and a custom inner loop, so we manage training by hand.

Run via lora/Wan2.1-Fun-V1.1-1.3B-Control_cartoon_sink_dmd.sh
"""
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            # ─── pick generator timestep + noise the target ───
            t_gen = sample_generator_timestep(denoising_step_list)
            sigma_gen = timestep_to_sigma(t_gen)
            noise_gen = torch.randn_like(target_latents)
            x_t_gen, _ = add_noise_flow(target_latents, sigma_gen, noise_gen)

            # ─── single student forward per step (WITH grad — kept for gen update) ───
            v_student = dit_forward(
                student_pipe.dit, x_t_gen, t_gen, prompt_embed,
                control_latents, ref_student, clip_student, device, dtype,
                use_gradient_checkpointing=args.use_gradient_checkpointing,
            )
            x_pred = velocity_to_x0(v_student, x_t_gen, sigma_gen)
