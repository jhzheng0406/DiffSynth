# DMD-recycle v2 — same student training as v1, but teacher LoRA is now the
# recycle-trained sink LoRA (latent-space error recycle on the recent ref).
#
# vs v1:
#   - TEACHER_LORA: sink_v2/step-1745  →  sink_recycle_v1/<latest>
#   - OUTPUT_DIR:   wan1.3b_dmd_recycle_v1  →  wan1.3b_dmd_recycle_v2
#   - All student-side hyperparams identical (apples-to-apples).
#
# Hypothesis: a teacher that's been trained robust to recent-ref drift gives
# the DMD student a stronger anchor distribution, so the student's own SVI
# recycle has less work to do (and edges/裙边 should stay sharper).
#
# Override the teacher checkpoint with:
#   TEACHER_STEP=875 bash <this-script>          # specific step
#   TEACHER_STEP=epoch-4 bash <this-script>      # specific epoch
#   TEACHER_LORA=path/to/lora.safetensors bash <this-script>

OUTPUT_DIR="./models/train/wan1.3b_dmd_recycle_v2_realdance"
TEACHER_DIR="./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_realdance_sink_recycle_v1"

# Resolve teacher LoRA:
#   TEACHER_LORA (full path)  > TEACHER_STEP (step/epoch tag) > auto-pick latest
...
accelerate launch \
    ...
    --dataset_metadata_path ./data/realdance_mariam/metadata.csv \
