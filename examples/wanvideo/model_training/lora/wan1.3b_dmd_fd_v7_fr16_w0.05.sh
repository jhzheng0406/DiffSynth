# DMD + FD-loss  v7:  fd_num_frames=16  +  fd_weight 0.03 → 0.05
#
# Conservative FD-weight variant (vs v6 = w0.1). Use if w0.1 turns out to over-pull.
# 0.05 contributes ~1.5× dmd_g — meaningful nudge without dominance.
#
# Code = train_dmd_fd_v3.py (same as v5, checkpoint-based VAE decode).
# Other hyperparams identical to v5.

OUTPUT_DIR="./models/train/wan1.3b_dmd_fd_v7_fr16_w0.05"
