# DMD + self-forcing rollout + FD-loss  v3  (re-tuned hyperparams).
#
# Code is SAME as v2 (train_dmd_fd_v2.py): normalized FD + cross-rank all_gather.
# Only hyperparams change:
#   lr  2e-6 / 4e-7  →  5e-6 / 5e-6   (match the "good" oneforcing run)
#   fd_weight 1.0    →  0.03           (match GAN's role as auxiliary, not dominator)
#
# Why v3 (not in-place update to v2):
#   Different lr can't cleanly resume v2 optimizer state. Different fd_weight =
#   different training objective. Cleanest is fresh dir for clean A/B.

OUTPUT_DIR="./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_sink_dmd_fd_v3"
...
[ -f "$FD_STATS" ] || { echo "[error] FD stats missing: $FD_STATS  (run precompute_fd_stats.py)"; exit 1; }

NUM_GPUS=${NUM_GPUS:-8}

accelerate launch \
    --num_processes=$NUM_GPUS --mixed_precision=bf16 \
    examples/wanvideo/model_training/train_dmd_fd_v2.py \
