# DMD + One-Forcing + Multi-Scale PatchGAN  v4  (softplus + stronger D).
#
# v2 finding: hinge ms_disc stalled — `gan_d_ms` stuck at ~0.060, `gan_g_ms`
# stuck at ~0 (D outputs saturated at hinge margin ±1, no gradient flow back
# to student).  msgan_v2 ≈ oneforcing visually.
#
# v4 fixes:
#   1. loss_type hinge → SOFTPLUS  — gives baseline log(2) pressure even at
#      logit=0, so G always feels signal and D never saturates.
#   2. base_ch 32 → 64             — 2× wider PatchD,more capacity to learn.
#   3. msgan_g/d_weight 0.03 → 0.05 — slightly stronger pull on student.
#
# Keep:  num_scales=2  (1× and 1/2×, full + mid-freq)
#        num_frames=4, vae_chunk=2  (memory budget tight)
#
# Code: same train_dmd_edge_gan.py (loss flagged via --msgan_loss_type).

OUTPUT_DIR="./models/train/wan1.3b_dmd_edge_gan_v1"
...
accelerate launch \
    ...
    examples/wanvideo/model_training/train_dmd_edge_gan.py \
    ...
    --msgan_g_weight 0.05 --msgan_d_weight 0.05 \
    --msgan_num_scales 2 --msgan_base_ch 64 --msgan_num_layers 3 \
    --msgan_num_frames 4 --msgan_vae_chunk 2 \
    --msgan_loss_type softplus \
