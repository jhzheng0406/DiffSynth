# LoRA training: cartoon style + reference_image input, NO EAD.
# Ablation control between cartoon.sh (no ref, no EAD) and cartoon_ead.sh
# (ref + EAD). Trains LoRA from scratch on base — same step count as the
# EAD variant, so the only diff vs cartoon_ead is: EAD on/off.
#
# Inputs:
...
NUM_GPUS=${NUM_GPUS:-2}
RESUME_FROM=${RESUME_FROM:-models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_with_ref/step-1990.safetensors}
...
accelerate launch \
  ...
  --output_path "./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_with_ref${OUT_SUFFIX}" \
  --lora_base_model "dit" \
