# Clean CONTROL for the "aug at DMD stage" paper claim.
#
#   teacher  = base + sink_noaug LoRA  (same as dmd_noaugteacher_asym)
#   student  = base + sink_noaug + LoRA (4-step, trained here)
#   mode     = off  +  --recent_aug_strength 0   →  ZERO aug anywhere
#
# Both teacher and student see CLEAN sink + CLEAN recent throughout (ref latent
# AND clip feature — uses the post-fix routing in train_dmd.py).
#
# Pairs with dmd_noaugteacher_asym (same teacher, asymmetric aug at DMD) to
# isolate the asym mechanism's contribution:
#   control vs asym  →  pure asymmetric-aug effect (teacher held constant)

OUTPUT_DIR="./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_dmd_noaug_control"
