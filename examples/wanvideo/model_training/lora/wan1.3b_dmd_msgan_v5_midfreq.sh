# DMD + One-Forcing + Multi-Scale PatchGAN  v5  (mid-freq ablation of v4).
#
# SAME as v4 (softplus, base_ch=64, g/d_weight=0.05) BUT with
# --msgan_skip_full_res → scales {1/2×, 1/4×} instead of {1×, 1/2×}.
#
# Purpose: paper ablation for v4. By removing the 1× scale (which catches
# high-freq texture) we isolate the mid/low-freq structural contribution.
# Direct A/B vs v4 reveals how much the 1× scale matters for the hand/dress-
# detail problem.  Expected: v4 wins on fine detail, v5 wins on body-part
# structure consistency.
#
# Code: same train_dmd_msgan_v2.py.

OUTPUT_DIR="./models/train/wan1.3b_dmd_msgan_v5_midfreq"
