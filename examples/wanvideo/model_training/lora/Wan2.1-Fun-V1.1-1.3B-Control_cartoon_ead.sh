# LoRA + EAD training: cartoon style + Helios-style reference noise.
# Clean ablation — trains LoRA from scratch on base (no --lora_checkpoint),
# same step count as the no-ref baseline (cartoon.sh), so the only diff vs
# that baseline is: + reference_image input  + EAD noise on ref_latents.
#
# Inputs:
...
NUM_GPUS=${NUM_GPUS:-2}
RESUME_FROM=${RESUME_FROM:-models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_ead_from_scratch/step-1990.safetensors}
...
accelerate launch \
  ...
  --output_path "./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_ead_from_scratch${OUT_SUFFIX}" \
  --lora_base_model "dit" \
