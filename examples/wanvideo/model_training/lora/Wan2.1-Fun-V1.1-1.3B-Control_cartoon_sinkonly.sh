# 1.3B LoRA, SINK ONLY — sink anchor, NO recent frame, NO augment.
#
#   --extra_inputs "control_video,sink_reference_image"   (no reference_image)
#   --recent_aug_strength 0                               (moot — no recent)
#
# WanVideoUnit_FunReference handles sink-only: reference_image=None → the ref
# latent is just the 1-frame sink (wan_video.py:583), CLIP falls back to sink.
#
# Ablation ladder (all same data / Dataset / hyperparams / steps):
#   chunked_noref : pose only            (no sink, no recent)
#   sinkonly      : pose + sink          (THIS — no recent)
#   sink_noaug    : pose + sink + recent (no augment)
#   sink_v2       : pose + sink + recent + augment(0.5)
#
...
if [ -n "$RESUME_FROM" ]; then
    ...
    RESUME_ARG=""
    echo "[fresh] training sink-only LoRA from base model"
fi
...
accelerate launch \
  ...
  --output_path "./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_sinkonly" \
  --lora_base_model "dit" \
  --lora_target_modules "q,k,v,o,ffn.0,ffn.2" \
  --lora_rank 32 \
  --extra_inputs "control_video,sink_reference_image" \
