# DMD + self-forcing rollout + FD-loss  (GAN-free One-Forcing variant).
#
# Replaces the GAN discriminator with Fréchet Distance in DINOv2 feature space.
# No adversarial training → no D/G dynamics to balance, more stable.
#
# Prerequisite (run once before training):
...
#
# Reference: github.com/Jiawei-Yang/FD-loss

OUTPUT_DIR="./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_sink_dmd_fd_1step_with_ema"
...
# Make sure the reference stats exist before launching
if [ ! -f "$FD_STATS" ]; then
    echo "[error] FD reference stats not found: $FD_STATS"
    echo "Run precompute_fd_stats.py first, e.g.:"
    echo "  python examples/wanvideo/model_training/precompute_fd_stats.py \\"
    echo "      --metadata_path ./data/cartoon_15s/metadata.csv \\"
    echo "      --output $FD_STATS"
    exit 1
fi
...
accelerate launch \
    ...
    examples/wanvideo/model_training/train_dmd_fd.py \
