# DMD + FD-loss  v5:  fd_num_frames=16  using train_dmd_fd_v3.py.
#
# Change vs v4 (which 137'd):
#   train_dmd_fd_v3.py replaces  torch.autograd.graph.save_on_cpu(...)
#   with  torch.utils.checkpoint.checkpoint(...)  around VAE decode + DINOv2.
#   → No more CPU RAM accumulation; activations recomputed in backward.
#
# First v5_fr16 attempt (chunk=0) hit CUDA OOM on H200 (140 GB) — the recompute
# stored the full 13-latent VAE activation in VRAM, on top of DiT grad-ckpt
# recompute that runs simultaneously in gen_loss.backward().  Fix:
#   --fd_vae_chunk 4   → decode 4 latent frames at a time
#                       caps VAE recompute peak to ~1/3 of full-decode
#   PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True → less fragmentation
#
# Other hyperparams: SAME as v3/v4.

OUTPUT_DIR="./models/train/wan1.3b_dmd_fd_v5_fr16"
...
# Reduce VRAM fragmentation — recommended by PyTorch when peak alloc is close to capacity.
