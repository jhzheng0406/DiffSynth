# 14B LoRA + sink + recent training (Helios-simplified).
# Same chunk-aware sink + recent training as the 1.3B sibling, on the 14B base.
# Each sample = one 49-frame chunk; sink = video[0] (clean), recent = video[s-1]
# (image-level aug applied).
#
# Prerequisite (run once, in videox env):
#   python data/cartoon_15s/prepare.py
#
# Memory: 14B + 49 frames + dual ref (T=2 latents) is heavier than the 14B
# baseline. If OOM:
...
NUM_GPUS=${NUM_GPUS:-4}
RESUME_FROM=${RESUME_FROM:-models/train/Wan2.1-Fun-V1.1-14B-Control_lora_cartoon_sink/step-600.safetensors}
...
accelerate launch \
  ...
  --output_path "./models/train/Wan2.1-Fun-V1.1-14B-Control_lora_cartoon_sink${OUT_SUFFIX}" \
  --lora_base_model "dit" \
