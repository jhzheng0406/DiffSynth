# DMD with the SINK-ONLY teacher (sink anchor, NO recent frame at all).
#
#   teacher  = base + sinkonly LoRA  (trained with sink only, no recent)
#   student  = base + sinkonly + LoRA (4-step, trained here)
#
# --no_recent → reference latent is sink-only (1 frame) for all models, matching
# how this teacher was trained. Tests whether a sink-only model distills into a
# good few-step student (no recent-frame chaining at inference either).

OUTPUT_DIR="./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_dmd_sinkonlyteacher"
TEACHER_LORA="./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_sinkonly/step-1745.safetensors"
