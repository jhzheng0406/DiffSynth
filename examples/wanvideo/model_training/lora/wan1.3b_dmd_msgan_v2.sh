# DMD + One-Forcing (cls_branch GAN) + Multi-Scale Patch GAN (hybrid).
#
# v1 was wrong: replaced cls_branch with weak PatchD → lost adversarial signal.
# v2: KEEP cls_branch AND add multi-scale pixel PatchD on top.
#
# Two complementary discriminators:
#   - cls_branch    on critic transformer features (semantic / structural)
#   - MultiScalePatchD on VAE-decoded pixels at 2 scales (1× and 1/2×)
#
# Memory: oneforcing baseline (~80GB) + msgan path (~10-15GB extra).
# Defaults conservative: scales=2, base_ch=32, num_frames=4, vae_chunk=2.

OUTPUT_DIR="./models/train/wan1.3b_dmd_msgan_v2"
