# DMD + Self-Forcing rollout + GAN  ("One-Forcing" port to our chunk+sink setup)
#
#   teacher  = base + sink_v2 LoRA (frozen, CFG-enhanced real score)
#   student  = base + sink_v2 + LoRA (trained, self-forcing rollout)
#   critic   = base + sink_v2 + LoRA (DMD critic, denoising + discriminator
#              shared backbone; cls_branch on layers [13,21,29])
#
# Three updates per step:
#   1. critic denoising MSE (DMD critic_loss)            × dfake_gen_update_ratio
#   2. GAN-D: cls_branch + critic features → log(D)      × dfake_gen_update_ratio
#   3. generator: dmd_loss + gan_g_weight * G_loss       × 1
#
# Reference: github.com/Aurora-edu/One-Forcing  (model/one_forcing.py)

OUTPUT_DIR="./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_oneforcing_1step_with_ema"
...
accelerate launch \
    ...
    examples/wanvideo/model_training/train_dmd_oneforcing.py \
