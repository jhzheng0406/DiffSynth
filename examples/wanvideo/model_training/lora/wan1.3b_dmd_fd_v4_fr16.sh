# DMD + FD-loss  v4:  fd_num_frames 8 → 16  (2× more samples per FD step).
#
# Reasoning:  v3's FD gradient direction is noisy because each step only sees
# 8 frames/rank × 8 ranks = 64 features for the 768-dim covariance estimate.
# Bumping to 16 frames → 128 features per step gives a cleaner gradient
# direction.  Fallback from fr24 (which OOM'd / hit container memory limit).
#
# Other hyperparams: SAME as v3
#   fd_weight=0.03,  lr=5e-6/5e-6,  num_inference_steps=1,
#   fd_queue_size=5000,  fd_feature_model=dinov2_vitb14
#
# Code: train_dmd_fd_v2.py (unchanged — just shell args differ)

OUTPUT_DIR="./models/train/wan1.3b_dmd_fd_v4_fr16"
