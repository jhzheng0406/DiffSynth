# DMD + One-Forcing GAN + Projected CLIP GAN — v4.
#
# vs v3: clip_d_lr 1e-4 → 2e-5 (5× smaller).
#   v3 D 学得过头 — gan_d_clip 60 步降到 0.001(D 完美区分),gan_g_clip
#   飙到 0.13(初始 5×),典型 GAN 失衡。fp32 head 不再有精度 floor,
#   小 lr 仍可学。目标:D 在 step ~200–300 才达稳态。
#
# 其他不变:hooks=4,8,11,12 / weights 0.03。

OUTPUT_DIR="./models/train/wan1.3b_dmd_clip_gan_v4"
