# DMD + One-Forcing GAN + Projected CLIP GAN — v3.
#
# Fixes vs v2 (which never moved d_loss_clip from baseline 0.041):
#   - Fix 1: CLIP D heads kept in fp32 (bf16 was rounding lr*grad to zero).
#   - Fix 2: Separate AdamW optimizer for D heads with lr=1e-4 (was sharing
#            critic_optimizer at 5e-6 — too small to escape AdamW init).
#
# Multi-layer hooks (4,8,11,12) preserved from v2; lower layers matter most
# (per Sauer 2021, late CLS is too semantic to discriminate).

OUTPUT_DIR="./models/train/wan1.3b_dmd_clip_gan_v3"
