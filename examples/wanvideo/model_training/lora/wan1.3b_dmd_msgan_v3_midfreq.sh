# DMD + One-Forcing (cls_branch) + Multi-Scale PatchD focused on MID-FREQ.
#
# v3 vs v2:  --msgan_skip_full_res  → pool once before the first D, so D scales
# are {1/2×, 1/4×} instead of {1×, 1/2×}.
#
# Rationale: 1× scale's receptive field overlaps with what cls_branch (on
# transformer features) already captures — local texture. The interesting
# extra signal is at MID-scale where each D patch covers ~80-280 px of the
# original image, i.e. body-part-sized: hands, dress hems, face structure.
#
# Code: same train_dmd_msgan_v2.py, just flag flipped.

OUTPUT_DIR="./models/train/wan1.3b_dmd_msgan_v3_midfreq"
