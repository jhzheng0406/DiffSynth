# LoRA fine-tune on Cartoon (200 samples) for Wan2.1-Fun-V1.1-1.3B-Control.
# Goal: large style shift (realistic → cartoon) so LoRA effect is visually
# obvious in A/B comparison. Same train mode as the humanvid version
# (video + pose only, no reference_image).
#
...
#   - If set   → continues LoRA from that ckpt; saved files are named
#                step-{N+200}, step-{N+400}, ... (global cumulative step)

NUM_GPUS=${NUM_GPUS:-2}
RESUME_FROM=${RESUME_FROM:-models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_resume_from_1200/step-1990.safetensors}
...
accelerate launch \
  ...
  --output_path "./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_from_1200${OUT_SUFFIX}" \
  --lora_base_model "dit" \
