# DMD + self-forcing rollout + FD-loss  v2 (paper-correct version).
#
# Fixes vs v1 (sink_dmd_fd_1step_with_ema):
#   1. NORMALIZED FD loss  fd_loss = fid / (fid.detach() + eps)
#   2. CROSS-RANK all_gather of features before computing FD
#   3. fd_weight 0.1 → 1.0  (with normalization, ~1 magnitude is correct scale)
#
# OLD dirs preserved untouched (..._fd_1step / ..._fd_1step_with_ema).
# This v2 writes to a brand-new dir for clean A/B.
#
# Reference stats are SAME as v1 (DINOv2 features unchanged):
#   ./data/cartoon_15s/fd_stats_dinov2.npz

OUTPUT_DIR="./models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_sink_dmd_fd_v2"
