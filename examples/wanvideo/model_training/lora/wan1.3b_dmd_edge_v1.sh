# DMD + One-Forcing (cls_branch GAN) + Sobel edge L1 loss.
#
# Keeps full oneforcing unchanged; adds direct edge supervision:
#     edge_loss = L1( Sobel(VAE_dec(x_pred)), Sobel(target_video) )
#     gen_loss  = dmd_g + 0.03*gan_g + edge_weight * edge_loss
#
# Why: prior aux losses (VGG perceptual, multi-scale PatchGAN) all failed
# to close the 5-6% Sobel gap between student and teacher. Sobel L1 is the
# most direct possible edge supervision — pixel-wise constraint on edge
# strength. No D to learn, no semantic feature averaging.
#
# Weight 0.05 chosen so edge_loss × weight ≈ dmd_g magnitude (~0.05).
# Tune to 0.1-0.2 if no effect, or down to 0.02 if it dominates.

OUTPUT_DIR="./models/train/wan1.3b_dmd_edge_v1"
...
accelerate launch \
    ...
    examples/wanvideo/model_training/train_dmd_edge.py \
