# Resume the 14B cartoon LoRA from step-1990. The previous run completed
# 2 epochs × dataset_repeat=10 = ~2000 steps. This shell adds another
# ~2000 steps on top.
#
# Caveats (same as 1.3B resume):
# - AdamW optimizer state is NOT restored.
# - The new train.log restarts the step counter from 1; treat new step N as
#   global step 1990 + N.
# - Output dir is new (_resume_from_1990) so the old step-200..step-1990
...
accelerate launch \
  ...
  --output_path "./models/train/Wan2.1-Fun-V1.1-14B-Control_lora_cartoon_resume_from_1990" \
  --lora_base_model "dit" \
  --lora_target_modules "q,k,v,o,ffn.0,ffn.2" \
  --lora_rank 32 \
  --extra_inputs "control_video" \
  --use_gradient_checkpointing \
  --lora_checkpoint "./models/train/Wan2.1-Fun-V1.1-14B-Control_lora_cartoon/step-1990.safetensors"
