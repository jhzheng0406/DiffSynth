# DMD-recycle v2 student — PORTRAIT, FREQUENCY-SPLIT recycle (ablation arm of #2).
#
# IDENTICAL to wan1.3b_dmd_recycle_v2_realdance_portrait.sh (same teacher, data,
# hyperparams, PLP, latent_cache) EXCEPT:
...
#   - separate OUTPUT_DIR
# So flat-vs-freqsplit is a clean A/B. The latent_cache is SHARED with the flat
# run (cached latents are independent of recycle mode → freq run reuses them).
#
# Diagnostic justified this (residual energy substantial+stable in both bands:
# k=2 → 58/42, k=4 → 33/67). First pass uses α_high>α_low to test whether the
# high band drives clarity; set both equal for a strength-matched control.

OUTPUT_DIR="./models/train/wan1.3b_dmd_recycle_v2_realdance_portrait_freqsplit"
...
accelerate launch \
  ...
  --aspect_crop \
    ...
    --error_alpha_low 0.25 --error_alpha_high 0.40 \
    --latent_cache ./data/realdance_portrait/latent_cache \
