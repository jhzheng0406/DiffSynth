# 14B teacher (Stage A) — recycle + PLP + aspect_crop, REAL DANCE (portrait).
# Scale-generality point: shows our method (sink + latent-space error recycle +
# PLP) works at 14B on the main benchmark domain (UBC + single-person 9:16),
# not just 1.3B. UBC is 3:4 → --aspect_crop on (cover + center-crop to 9:16,
# matches inference's crop_and_resize).
#
...
accelerate launch \
  ...
  --output_path "./models/train/Wan2.1-Fun-V1.1-14B-Control_lora_realdance_portrait_sink_recycle" \
  --lora_base_model "dit" \
