"""
DMD2 + ONE-FORCING (cls_branch GAN) + PROJECTED CLIP GAN (Sauer 2021).

Keeps One-Forcing's cls_branch (D on critic transformer features) intact.
Adds a SECOND adversarial discriminator that operates in pretrained CLIP
image-feature space, bringing billion-scale visual knowledge into the
adversarial signal — addressing the structural completeness problem of
1-step distillation.

Two parallel adversarial heads:
  A. cls_branch     — D on critic features (semantic / structural, in-domain)
  B. ClipDiscriminator — D on CLIP CLS-token features (broad pretrained prior)

Per step:
  1. Critic denoising MSE                              × N
  2. GAN-D update: cls_branch (latent path) AND CLIP-D (pixel→CLIP path).
     Both backward together.                            × N
  3. Generator update: dmd_g + gan_g_cls + gan_g_clip   × 1

Memory: CLIP-D path needs VAE-decoded pixels.  GAN-G path uses
torch.utils.checkpoint to bound activation memory.  Frames subsampled to
--clip_num_frames per step.
"""
...
from clip_discriminator import ClipDiscriminator, clip_gan_g_loss, clip_gan_d_loss
...
def encode_batch(pipe, batch, dtype, device, no_recent=False):
    ...
    return dict(
        ...
        target_video=target_video,                # pixels in [-1,1] for CLIP-D path
...
def get_parser():
    ...
    # ─── Projected CLIP GAN (second discriminator on CLIP feature space) ───
    p.add_argument("--clip_g_weight", type=float, default=0.03,
                   help="Weight on CLIP-D's generator loss in gen_loss. 0=disabled.")
    p.add_argument("--clip_d_weight", type=float, default=0.03,
                   help="Weight on CLIP-D's discriminator loss in D update.")
    p.add_argument("--clip_model_name", default="openai/clip-vit-base-patch32",
                   help="HF identifier for CLIP image encoder. Try clip-vit-large-patch14 for stronger D.")
    p.add_argument("--clip_d_hidden", type=int, default=512,
                   help="Hidden dim of trainable D head MLP.")
    p.add_argument("--clip_d_layers", type=int, default=2,
                   help="Number of layers in trainable D head MLP.")
    p.add_argument("--clip_hook_layers", default="4,8,11,12",
                   help="CLIP hidden_states indices to hook (comma-separated). "
                        "One D head per layer; logits averaged. ViT-B has 12 blocks → idx 0..12.")
    p.add_argument("--clip_d_lr", type=float, default=1e-4,
                   help="Separate AdamW lr for CLIP D heads (kept large because their "
                        "gradient magnitudes are smaller than the critic's and they're fp32).")
    p.add_argument("--clip_num_frames", type=int, default=8,
                   help="Sample N frames per step for CLIP path (memory bound).")
    p.add_argument("--clip_vae_chunk", type=int, default=2,
                   help="VAE-decode chunk size along latent time axis (for G step with grad).")
...
def main():
    ...
    # ---------------- Projected CLIP discriminator (multi-layer) ----------------
    hook_layers = tuple(int(x) for x in args.clip_hook_layers.split(","))
    clip_disc = ClipDiscriminator(
        clip_model_name=args.clip_model_name,
        hidden_dim=args.clip_d_hidden,
        num_layers=args.clip_d_layers,
        hook_layers=hook_layers,
        dtype=dtype,
    ).to(device)
    # Only the per-layer head params are trainable; CLIP encoder is frozen.
    clip_disc.train()
    clip_head_params = list(clip_disc.heads.parameters())
    if is_main:
        n_clip = sum(p.numel() for p in clip_disc.parameters())
        n_clip_train = sum(p.numel() for p in clip_head_params)
        print(f"[setup] clip_disc: {n_clip/1e6:.1f}M total / {n_clip_train/1e6:.2f}M trainable "
              f"(model={args.clip_model_name}, hook_layers={hook_layers}, "
              f"frames/step={args.clip_num_frames})")
    ...
    # critic_optimizer drives critic + cls_branch (both bf16, both need critic_lr).
    critic_optimizer  = torch.optim.AdamW(critic_params + cls_params,
                                          lr=args.learning_rate_critic, weight_decay=1e-2)
    # CLIP D heads have a SEPARATE optimizer with much higher lr — they're
    # fp32 standalone MLPs that don't benefit from the critic's backbone
    # gradient flow, so they need lr ~1e-4 (20× critic_lr) to actually learn.
    clip_d_optimizer = torch.optim.AdamW(clip_head_params, lr=args.clip_d_lr, weight_decay=0.0)
    if is_main:
        print(f"[setup] clip_d_optimizer: lr={args.clip_d_lr} ({len(clip_head_params)} param tensors)")
    ...
    if args.resume_student_from:
        ...
        if os.path.isfile(state_path):
            ...
            # clip_d_optimizer was added later; tolerate older checkpoints.
            if "clip_d_optimizer" in ckpt:
                try:
                    clip_d_optimizer.load_state_dict(ckpt["clip_d_optimizer"])
                except (ValueError, KeyError) as e:
                    if is_main:
                        print(f"[resume] clip_d_optimizer state mismatch ({e}) — cold start.")
            elif is_main:
                print("[resume] no clip_d_optimizer in ckpt (older run) — cold start.")
            if is_main:
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            for _ in range(args.dfake_gen_update_ratio):
                ...
                # Critic loss doesn't flow into clip heads, but we still zero
                # them to keep accumulated grads clean.
                clip_d_optimizer.zero_grad()
            ...
            for _ in range(args.dfake_gen_update_ratio):
                ...
                # ─── CLIP-D path ─────────────────────────────────────
                # Decode fake & real to pixels (no grad to student/VAE for D step),
                # sample N frames, pass through CLIP-D head.
                if args.clip_d_weight > 0.0:
                    T_lat = x_pred.shape[2]
                    n_clip = min(args.clip_num_frames, T_lat)
                    idx_clip = torch.randperm(T_lat, device=device)[:n_clip]
                    with torch.no_grad():
                        # decode subset of latent slices to pixels (chunked)
                        x_lat_sub = x_pred.detach().index_select(2, idx_clip)
                        chunk_sz = max(1, args.clip_vae_chunk)
                        fake_pieces = []
                        for ci in range(0, x_lat_sub.shape[2], chunk_sz):
                            sub = x_lat_sub[:, :, ci:ci + chunk_sz]
                            fake_pieces.append(student_pipe.vae.decode(
                                sub.to(dtype), device=device).to(dtype).clamp(-1, 1))
                        fake_pix_d = torch.cat(fake_pieces, dim=2)         # [B,3,T_pix,H,W]
                        # real: subsample matching pixel frames from target_video
                        pix_idx_d = torch.clamp(idx_clip * 4, 0, cond["target_video"].shape[2] - 1)
                        real_pix_d = cond["target_video"].index_select(2, pix_idx_d).clamp(-1, 1)
                    ...
                    def _flat(v):
                        B, C, Tv, H, W = v.shape
                        return v.permute(0, 2, 1, 3, 4).reshape(B * Tv, C, H, W)

                    fake_logit_clip = clip_disc(_flat(fake_pix_d))
                    real_logit_clip = clip_disc(_flat(real_pix_d))
                    d_loss_clip = clip_gan_d_loss(real_logit_clip, fake_logit_clip) * args.clip_d_weight
                else:
                    d_loss_clip = fake_logit_d.new_zeros(())

                d_loss = d_loss_cls + d_loss_clip
                critic_optimizer.zero_grad()
                clip_d_optimizer.zero_grad()
                d_loss.backward()
                # Sync gradients across ranks separately by param group.
                all_reduce_grads(critic_params + cls_params, num_proc)
                all_reduce_grads(clip_head_params, num_proc)
                critic_optimizer.step()
                clip_d_optimizer.step()
            ...
            # ─── CLIP-G path ─────────────────────────────────────
            # VAE-decode x_pred (with grad through VAE, checkpointed) → CLIP-D → G loss.
            # Subsample N frames for memory.
            if args.clip_g_weight > 0.0:
                T_lat = x_pred.shape[2]
                n_clip_g = min(args.clip_num_frames, T_lat)
                idx_clip_g = torch.randperm(T_lat, device=device)[:n_clip_g]
                x_lat_sub_g = x_pred.index_select(2, idx_clip_g)   # with grad

                def _decode_clamp(x_lat):
                    return student_pipe.vae.decode(
                        x_lat.to(dtype), device=device,
                    ).to(dtype).clamp(-1, 1)

                chunk_g = max(1, args.clip_vae_chunk)
                fake_pieces_g = []
                for ci in range(0, x_lat_sub_g.shape[2], chunk_g):
                    sub = x_lat_sub_g[:, :, ci:ci + chunk_g]
                    fake_pieces_g.append(torch.utils.checkpoint.checkpoint(
                        _decode_clamp, sub, use_reentrant=False,
                    ))
                fake_pix_g = torch.cat(fake_pieces_g, dim=2)   # [B,3,T,H,W] with grad
                Bg, Cg, Tg, Hg, Wg = fake_pix_g.shape
                fake_logit_clip_g = clip_disc(
                    fake_pix_g.permute(0, 2, 1, 3, 4).reshape(Bg * Tg, Cg, Hg, Wg)
                )
                g_loss_clip = clip_gan_g_loss(fake_logit_clip_g) * args.clip_g_weight
            else:
                g_loss_clip = fake_logit_g.new_zeros(())

            g_loss = g_loss_cls + g_loss_clip
            ...
            if is_main:
                pbar.set_postfix(
                    ...
                    gg=f"{g_loss_cls.item():.4f}", gd=f"{d_loss_cls.item():.4f}",
                    cg=f"{float(g_loss_clip):.4f}", cd=f"{float(d_loss_clip):.4f}",
                    t=t_gen,
                )
                log_file.write(
                    ...
                    f"gan_g_cls={g_loss_cls.item():.6f} gan_d_cls={d_loss_cls.item():.6f} "
                    f"gan_g_clip={float(g_loss_clip):.6f} gan_d_clip={float(d_loss_clip):.6f} "
            ...
            if global_step % args.save_steps == 0:
                ...
                if is_main:
                    ...
                    torch.save({
                        ...
                        "clip_d_optimizer":  clip_d_optimizer.state_dict(),
