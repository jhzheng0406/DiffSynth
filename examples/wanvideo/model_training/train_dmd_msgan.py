"""
DMD2 + SELF-FORCING ROLLOUT + MULTI-SCALE PATCH GAN  (pixel-space).

Replaces One-Forcing's ClsBranch-on-transformer-features GAN with a
proper Pix2PixHD-style multi-scale PatchGAN operating directly on
VAE-decoded pixels at K spatial scales (default 3: 1×, 1/2×, 1/4×).

Why:
  * cls_branch shares the discriminator's input space with DMD critic
    (both look at transformer features) → potentially redundant signals.
  * Multi-scale pixel PatchD lives in a DIFFERENT feature space (pixels) and
    can be made multi-scale literally — full-res D catches local texture,
    low-res D catches global structure.

Each step:
  1. Critic denoising MSE                                       × N
  2. GAN-D update: VAE_dec(x_pred.detach()) vs target_video      × N
                   feed to MultiScalePatchD, hinge D loss
  3. Generator: DMD + msgan_g_weight * msgan_g_loss              × 1
                fake pixels via VAE_dec(x_pred) (grad through VAE)

VAE decode wrapped in torch.utils.checkpoint with chunked time-axis
(same pattern as train_dmd_oneforcing_hf.py / train_dmd_fd_v3.py).
"""
...
def encode_batch(pipe, batch, dtype, device, no_recent=False):
    ...
    return dict(
        ...
        target_video=target_video,                # pixels in [-1,1] for D real input
...
def get_parser():
    ...
    # ─── Multi-scale Patch GAN ───
    p.add_argument("--msgan_g_weight", type=float, default=0.03,
                   help="Weight of multi-scale GAN G-loss added to generator's total loss.")
    p.add_argument("--msgan_d_weight", type=float, default=0.03,
                   help="Weight on the multi-scale PatchD update.")
    p.add_argument("--msgan_num_scales", type=int, default=3,
                   help="Number of spatial scales (each is 2x avg-pool of the previous).")
    p.add_argument("--msgan_base_ch", type=int, default=64,
                   help="Base channel width of each PatchD.")
    p.add_argument("--msgan_num_layers", type=int, default=3,
                   help="Number of stride-2 blocks in each PatchD.")
    p.add_argument("--msgan_num_frames", type=int, default=8,
                   help="How many latent slices to sample per step for the GAN path "
                        "(full 13 = use whole chunk; less = save memory).")
    p.add_argument("--msgan_vae_chunk", type=int, default=4,
                   help="VAE-decode latent chunk size for the GAN path. 0 = single call.")
...
def main():
    ...
    if is_main:
        ...
        print(f"[setup] MultiScalePatchD: {n_d/1e6:.1f}M params, "
              f"scales={args.msgan_num_scales}, base_ch={args.msgan_base_ch}, "
              f"layers={args.msgan_num_layers}")
    ...
    if is_main:
        ...
        print(f"[setup] msgan disc params:        {sum(p.numel() for p in disc_params)/1e6:.1f}M")

    student_optimizer = torch.optim.AdamW(student_params, lr=args.learning_rate_student, weight_decay=1e-2)
    # critic_optimizer drives BOTH the denoising critic AND the multi-scale disc
    # (parallel to One-Forcing's cls_branch grouping).
    critic_optimizer  = torch.optim.AdamW(critic_params + disc_params,
    ...
    if args.resume_student_from:
        # MultiScalePatchD state (..._disc.pt next to student LoRA)
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            for _ in range(args.dfake_gen_update_ratio):
                ...
                all_reduce_grads(critic_params + disc_params, num_proc)   # sync
                critic_optimizer.step()

            # ─── GAN-D update × N  (multi-scale pixel-space PatchD) ───
            # Decode latent → pixels for both fake and real, feed to MultiScalePatchD.
            # Memory: VAE decode wrapped in no_grad (no grad path needed here since
            # we only update D params, not the student). VGG-style chunked decode
            # to keep peak VRAM bounded.
            T_lat = x_pred.shape[2]
            n_msg = min(args.msgan_num_frames, T_lat)
            for _ in range(args.dfake_gen_update_ratio):
                # Sample which latent slices to use for this D step
                idx = torch.randperm(T_lat, device=device)[:n_msg]
                x_pred_sub = x_pred.detach().index_select(2, idx)
                # Map latent idx → corresponding pixel-frame indices (approx via 4x temporal)
                pix_idx = torch.clamp(idx * 4, 0, cond["target_video"].shape[2] - 1)
                with torch.no_grad():
                    real_pix = cond["target_video"].index_select(2, pix_idx).clamp(-1, 1)
                    # VAE decode fake (chunked to bound memory)
                    chunk = args.msgan_vae_chunk if args.msgan_vae_chunk > 0 else x_pred_sub.shape[2]
                    fake_pieces = []
                    for ci in range(0, x_pred_sub.shape[2], chunk):
                        sub = x_pred_sub[:, :, ci:ci+chunk]
                        fake_pieces.append(student_pipe.vae.decode(
                            sub.to(dtype), device=device).to(dtype).clamp(-1, 1))
                    fake_pix = torch.cat(fake_pieces, dim=2)
                    # Match temporal dim
                    T_min = min(fake_pix.shape[2], real_pix.shape[2])
                    fake_pix = fake_pix[:, :, :T_min]
                    real_pix = real_pix[:, :, :T_min]

                # Reshape video → independent frames: [B, 3, T, H, W] → [B*T, 3, H, W]
                def _vid2frames(v):
                    B, C, T, H, W = v.shape
                    return v.permute(0, 2, 1, 3, 4).reshape(B * T, C, H, W)

                fake_logits = ms_disc(_vid2frames(fake_pix))     # list of K patch maps
                real_logits = ms_disc(_vid2frames(real_pix))
                d_loss = msgan_d_loss(fake_logits, real_logits) * args.msgan_d_weight

                critic_optimizer.zero_grad()
                d_loss.backward()
                all_reduce_grads(critic_params + disc_params, num_proc)
                critic_optimizer.step()
            ...
            # GAN-G: push student via the (frozen-this-step) multi-scale PatchD.
            # Decode x_pred with grad (checkpoint to bound memory), feed to ms_disc,
            # hinge G loss.
            idx_g = torch.randperm(T_lat, device=device)[:n_msg]
            x_pred_sub_g = x_pred.index_select(2, idx_g)        # WITH grad
            ...
            chunk = args.msgan_vae_chunk if args.msgan_vae_chunk > 0 else x_pred_sub_g.shape[2]
            fake_pieces_g = []
            for ci in range(0, x_pred_sub_g.shape[2], chunk):
                sub = x_pred_sub_g[:, :, ci:ci + chunk]
                fake_pieces_g.append(torch.utils.checkpoint.checkpoint(
                    _decode, sub, use_reentrant=False,
                ))
            fake_pix_g = torch.cat(fake_pieces_g, dim=2)
            # [B,3,T,H,W] → [B*T,3,H,W]
            Bg, Cg, Tg, Hg, Wg = fake_pix_g.shape
            fake_logits_g = ms_disc(fake_pix_g.permute(0, 2, 1, 3, 4).reshape(Bg * Tg, Cg, Hg, Wg))
            g_loss = msgan_g_loss(fake_logits_g) * args.msgan_g_weight
            ...
            if global_step % args.save_steps == 0:
                ...
                if is_main:
                    ...
                    print(f"[save] {student_path} + ema:{ema_path} + {disc_path} + {state_path}")
