"""
DMD2 + SELF-FORCING ROLLOUT + FD-LOSS  v3  (memory fix vs v2):

  CHANGED:
    VAE-decode-for-FD now uses `torch.utils.checkpoint` instead of
    `torch.autograd.graph.save_on_cpu`.  The save_on_cpu version offloaded
    every intermediate activation to host RAM and accumulated until backward
    consumed them — at fd_num_frames >= T_lat (=13) that pushed CPU RAM past
    container limits → SIGKILL (137).  Checkpoint recomputes activations in
    backward instead, costing one extra VAE forward but no CPU RAM growth.

  Also added:
    --fd_vae_chunk N   decode latents in chunks of N along time (default 0 = no
                       chunking; useful if VRAM still tight at large fr).

  UNCHANGED from v2:  normalized FD, cross-rank all_gather, fd_weight role,
  reference stats file, every other training detail.

Reference stats:
    ./data/cartoon_15s/fd_stats_dinov2.npz  (same as v1/v2)
"""
...
from fd_loss_v2 import (
    ...
    compute_fd_loss_normalized, precompute_sigma_ref_sqrt, load_fd_stats,
    diff_all_gather,
)
...
def get_parser():
    ...
    p.add_argument("--fd_vae_chunk", type=int, default=0,
                   help="Decode latents in chunks of this many along time. "
                        "0 (default) = single decode call.  Use small values "
                        "(e.g. 4) if VRAM tight at large fd_num_frames.")
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            # then VAE decode. Uses torch.utils.checkpoint: activations are NOT
            # stored — they get recomputed during backward. Costs ~one extra
            # VAE forward but no CPU RAM accumulation (was the 137 OOM cause).
            ...
            def _vae_to_feats(x_lat):
                # Wan VAE decode: [B,C,T_lat,H_lat,W_lat] → [B,3,T,H,W] in [-1,1]
                decoded = student_pipe.vae.decode(x_lat.to(dtype),
                                                  device=device).to(dtype)
                pix = ((decoded.clamp(-1, 1) + 1) / 2)                  # [B,3,T,H,W]
                pix = pix.permute(0, 2, 1, 3, 4)                        # [B,T,3,H,W]
                return fd_extractor(pix)                                # [B*T, D]

            chunk = args.fd_vae_chunk
            if chunk and chunk > 0 and chunk < x_pred_sub.shape[2]:
                feat_chunks = []
                for ci in range(0, x_pred_sub.shape[2], chunk):
                    sub = x_pred_sub[:, :, ci:ci + chunk]
                    feat_chunks.append(torch.utils.checkpoint.checkpoint(
                        _vae_to_feats, sub, use_reentrant=False,
                    ))
                new_feats_local = torch.cat(feat_chunks, dim=0)
            else:
                new_feats_local = torch.utils.checkpoint.checkpoint(
                    _vae_to_feats, x_pred_sub, use_reentrant=False,
                )
            # Pool features across ranks(differentiable: grad routes back to
            # local portion only). Big win for stable FD statistics.
            new_feats = diff_all_gather(new_feats_local)                # [B*T*W, D]
            ...
            # Enqueue gathered features so every rank's queue stays consistent
            ...
            if is_main:
                pbar.set_postfix(
                    ...
                    fd=f"{fd_g_loss.item():.3f}", fdR=f"{float(fd_raw_val):.1f}",
                    q=int(fd_queue.count.item()), t=t_gen,
                )
                log_file.write(
                    f"{time.strftime('%H:%M:%S')} epoch={epoch} step={global_step} "
                    f"t_gen={t_gen} gen_loss={gen_loss.item():.6f} "
                    f"dmd_g={dmd_g_loss.item():.6f} fd_norm={fd_g_loss.item():.6f} fd_raw={float(fd_raw_val):.2f} "
