"""
DMD2 + ONE-FORCING (GAN) + HIGH-FREQUENCY PIXEL AUX LOSS.

Adds to the GAN-baseline (train_dmd_oneforcing.py) a per-frame Laplacian
high-pass loss between student-decoded pixels and the ground-truth video:

    hf_loss = MSE( Laplacian(VAE_dec(x_pred)), Laplacian(target_video) )
    gen_loss = dmd_g + gan_g_weight * gan_g + highfreq_weight * hf_loss

Rationale (from blur analysis): GAN handles textures across most of the
frame but corners stay slightly blurry; a direct pixel-space high-pass
target pushes the student to also match the true distribution of edges
and fine detail.

Memory: VAE decode of x_pred for HF is wrapped in torch.utils.checkpoint
(activations recomputed during backward), same pattern as train_dmd_fd_v3.
Chunked via --hf_vae_chunk N if VRAM tight.

Other branches (critic, GAN-D, GAN-G) unchanged from train_dmd_oneforcing.
"""
...
# ===========================================================================
# Laplacian (high-pass) — per-channel, 2D over (H, W), batched across (B, T)
# ===========================================================================
...
def laplacian_highpass(x: torch.Tensor) -> torch.Tensor:
    """Apply Laplacian (high-pass) filter per channel.
    x: [B, C, T, H, W]   →   same shape, edges/highfreq energy.
    """
    B, C, T, H, W = x.shape
    kernel = _LAPLACIAN_KERNEL.to(dtype=x.dtype, device=x.device)
    kernel = kernel.view(1, 1, 3, 3).expand(C, 1, 3, 3).contiguous()
    x_flat = x.permute(0, 2, 1, 3, 4).reshape(B * T, C, H, W)
    out = F.conv2d(x_flat, kernel, padding=1, groups=C)
...
def get_parser():
    ...
    # ─── High-frequency pixel aux loss ───
    p.add_argument("--highfreq_weight", type=float, default=0.0,
                   help="Weight on MSE(Laplacian(VAE_dec(x_pred)), "
                        "Laplacian(target_video)). 0 = off.")
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            # ─── High-frequency pixel aux loss ──────────────────────────────
            # VAE-decode x_pred, Laplacian high-pass, MSE against target_video.
            # checkpoint() recomputes activations in backward (no CPU offload).
            if args.highfreq_weight > 0.0:
                def _vae_decode(x_lat):
                    return student_pipe.vae.decode(
                        x_lat.to(dtype), device=device,
                    ).to(dtype).clamp(-1, 1)

                chunk = args.hf_vae_chunk
                if chunk and chunk > 0 and chunk < x_pred.shape[2]:
                    fake_pieces = []
                    for ci in range(0, x_pred.shape[2], chunk):
                        sub = x_pred[:, :, ci:ci + chunk]
                        fake_pieces.append(torch.utils.checkpoint.checkpoint(
                            _vae_decode, sub, use_reentrant=False,
                        ))
                    fake_pixels = torch.cat(fake_pieces, dim=2)
                else:
                    fake_pixels = torch.utils.checkpoint.checkpoint(
                        _vae_decode, x_pred, use_reentrant=False,
                    )

                with torch.no_grad():
                    real_pixels = cond["target_video"].clamp(-1, 1)
                    # Crop temporal dim if VAE decoder produced fewer frames
                    # than target_video (can happen at chunk boundaries).
                    T_min = min(fake_pixels.shape[2], real_pixels.shape[2])
                fake_hf = laplacian_highpass(fake_pixels[:, :, :T_min])
                real_hf = laplacian_highpass(real_pixels[:, :, :T_min])
                hf_loss = F.mse_loss(fake_hf, real_hf)
            else:
