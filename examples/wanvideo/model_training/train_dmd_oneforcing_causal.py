"""
DMD2 + SELF-FORCING ROLLOUT + GAN  ("One-Forcing" port).

Builds on train_dmd_rollout.py and adds the One-Forcing GAN branch
(Aurora-edu/One-Forcing, model/one_forcing.py):

  * Discriminator `ClsBranch` hangs off the CRITIC's intermediate features
    (layers [13,21,29] by default). Light cross-attention + MLP → logit.
  * Each training step has THREE updates:
      1. Critic denoising MSE (existing DMD critic_loss)            × N
      2. *** GAN-D step *** : critic + cls_branch discriminate
         noised student outputs (fake) vs noised ground-truth (real). × N
      3. Generator: DMD distribution-matching grad + gan_g_loss     × 1
  * Loss = dmd_loss + gan_g_weight * softplus(-fake_logit)
       D    = gan_d_weight * [ softplus(-real_logit) + softplus(fake_logit) ]
    Default gan_g_weight = gan_d_weight = 0.03 (One-Forcing defaults).

Run via lora/Wan2.1-Fun-V1.1-1.3B-Control_cartoon_sink_dmd_oneforcing.sh
"""
...
from diffsynth.models.wan_video_dit import set_block_causal_attention
from peft import LoraConfig, inject_adapter_in_model
from safetensors.torch import save_file


# Causal-AR (One-Forcing / Self-Forcing) baseline: build+attach the block-causal
# mask to the STUDENT dit ONCE, the first time we see the real chunk shapes.
# Bidirectional teacher distills into a causal student (CausVid recipe); critic
# and teacher stay bidirectional. Token layout is [ref(T_ref) | target(f)].
_CAUSAL_STATE = {"done": False}
def ensure_student_causal(student_dit, target_shape, reference_latents, block_frames, is_main=False):
    if _CAUSAL_STATE["done"]:
        return
    ps = student_dit.patch_size                      # (1, 2, 2) for Wan
    _, _, Tl, Hl, Wl = target_shape
    f = Tl // ps[0]; h = Hl // ps[1]; w = Wl // ps[2]
    if reference_latents is None:
        T_ref = 0
    elif reference_latents.dim() == 5:
        T_ref = reference_latents.shape[2]
    else:
        T_ref = 1
    set_block_causal_attention(student_dit, T_ref, f, h, w, block_frames=block_frames)
    _CAUSAL_STATE["done"] = True
    if is_main:
        print(f"[causal] block-causal mask on student: T_ref={T_ref} f={f} "
              f"h={h} w={w} block_frames={block_frames} (seq={(T_ref+f)*h*w})", flush=True)
...
def get_parser():
    ...
    p.add_argument("--causal_block_frames", type=int, default=1,
                   help="Block-causal granularity in LATENT frames (1 latent frame "
                        "≈ 4 real frames). 1 = strict per-latent-frame causal AR "
                        "(One-Forcing/Self-Forcing). The student attends only to the "
                        "ref prefix + earlier-or-equal causal blocks; teacher/critic "
                        "stay bidirectional.")
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            # ─── attach block-causal mask to the student (once, on first batch) ───
            ensure_student_causal(student_pipe.dit, tuple(target_latents.shape),
                                  ref_student, args.causal_block_frames, is_main=is_main)
