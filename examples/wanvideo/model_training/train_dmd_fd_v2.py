"""
DMD2 + SELF-FORCING ROLLOUT + FD-LOSS  v2  (fixes vs v1):

  1. NORMALIZED FD loss:  fd_loss = fid / (fid.detach() + eps)   ← paper trick
     so the loss magnitude stays ≈ 1 regardless of how big FD is.
  2. CROSS-RANK all_gather:  features pooled across all GPUs before computing
     FD → much more stable mu/sigma statistics each step.
  3. Default fd_weight bumped to 1.0  (with normalization, no scale concern).

v1 (train_dmd_fd.py) is kept for reproducibility; do NOT mix output dirs.

Reference stats (mu_ref, sigma_ref) — same file as v1:
    ./data/cartoon_15s/fd_stats_dinov2.npz

Run via lora/Wan2.1-Fun-V1.1-1.3B-Control_cartoon_sink_dmd_fd_v2.sh
"""
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            with torch.autograd.graph.save_on_cpu(pin_memory=False):
                ...
                new_feats_local = fd_extractor(pixels)                  # [B*T, D] local
