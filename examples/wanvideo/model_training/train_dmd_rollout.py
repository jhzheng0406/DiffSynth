"""
DMD2 distillation with SELF-FORCING ROLLOUT (Causal Forcing++ Stage-3 style).

Difference from train_dmd.py:
  * GENERATOR PATH: instead of noising the GT to a single random timestep and
    doing one student forward, we ROLL OUT the student through the full
    denoising_step_list starting from PURE NOISE. A random "exit" index is
    drawn; all steps before the exit are run under torch.no_grad with re-noising
    between steps, the exit step is run WITH grad. The exit output is x_pred.
  * Train ≡ inference: the student sees its own intermediate states (not
    GT-noised samples), which directly fixes the 2-step blur from train/
    inference distribution mismatch.
  * Cost: ~same as single-forward training, because only one step has grad
    (random-exit trick). The other rollout steps are no_grad ≈ cheap.

Reference: thu-ml/Causal-Forcing  pipeline/bidirectional_training.py
           inference_with_trajectory (random-exit-flag rollout).

Critic path unchanged: critic_loss still uses x_pred.detach() — the rollout
just changes WHERE x_pred comes from (rollout exit instead of GT-noising).

Run via lora/Wan2.1-Fun-V1.1-1.3B-Control_cartoon_sink_dmd_rollout.sh
"""
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            gen_loss = compute_dmd_loss(x_pred, grad)
            ...
            if is_main:
                pbar.set_postfix(g=f"{gen_loss.item():.4f}", c=f"{critic_loss.item():.4f}", t=t_gen)
                log_file.write(
            ...
            if global_step % args.save_steps == 0:
                ...
                if is_main:
                    ...
                    print(f"[save] {student_path}  +  {critic_path}  +  {state_path}")
