...
@torch.no_grad()
def encode_batch(pipe, batch, dtype, device, no_recent=False, plp=False, cache_dir=None):
    ...
    # First epoch populates; later epochs / reruns load tensors. Keyed by
    # (uid, plp, no_recent, H, W); clear the dir if the base VAE/T5 changes. ----
    _cache_path = None
    if cache_dir is not None and batch.get("uid") is not None:
        import hashlib
        _w, _h = batch["sink_reference_image"][0].size
        _key = hashlib.md5(f'{batch["uid"]}|plp{int(plp)}|nr{int(no_recent)}'.encode()).hexdigest()[:16]
    ...
    if plp and batch.get("recent_window") is not None:
        ...
        plp_recent = pipe.vae.encode(rw, device=device).to(dtype=dtype)[:, :, -1:]
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            cond = encode_batch(student_pipe, batch, dtype, device, no_recent=args.no_recent, plp=args.plp, cache_dir=args.latent_cache)
