"""
DMD2 + ONE-FORCING (cls_branch GAN) + SR-ANCHOR + SVI-STYLE ERROR RECYCLE.

Combines two orthogonal modifications on top of the oneforcing baseline:

  A. SR-anchor: GAN-D's "real" target = unsharp-mask(real_video). Pulls
     student toward sharper-than-real distribution. Set via --sr_amount.
     DMD path unchanged (still teacher-anchored).

  B. SVI recycle: error replay buffer + self-correcting loss to make student
     robust to drift in the recent_reference latent. Set via --error_alpha
     and --sc_weight. DMD/GAN paths unchanged in semantics.

A and B touch different parts of the loss landscape (A: GAN target,
B: conditioning + extra loss), so they compose. With sr_amount=0.5 and
recycle defaults, expect single-frame Sobel +20-30% AND long-form drift
reduction ~25%.

Below: SVI mechanism description (original).
...
# ===========================================================================
# SR-anchor: unsharp mask helpers (GAN-D's enhanced-real target)
# ===========================================================================
class _encode_batch_state:
    """Holds SR-anchor hyperparams readable from encode_batch (cross-module)."""
    sr_amount = 0.0
    sr_sigma  = 1.2


def _gaussian_kernel_1d(sigma: float, ksize=None):
    if ksize is None:
...
def _gaussian_blur_video(x: torch.Tensor, sigma: float = 1.2) -> torch.Tensor:
    """Per-frame separable 2D Gaussian blur. x: [B, C, T, H, W]."""
    B, C, T, H, W = x.shape
...
def _unsharp_mask_video(x: torch.Tensor, sigma: float = 1.2, amount: float = 1.0) -> torch.Tensor:
    """enhanced = x + amount * (x - blur(x)), clamped to [-1, 1]."""
...
def encode_batch(pipe, batch, dtype, device, no_recent=False):
    ...
    # SR-anchor: enhanced (sharpened) version for GAN-D real target.
    # DMD path stays on raw target_latents.
    ...
    return dict(
        ...
        enhanced_target_latents=enhanced_target_latents,   # SR-anchor: GAN-D's "real"
...
def get_parser():
    ...
    p.add_argument("--sr_amount", type=float, default=0.0,
                   help="Unsharp-mask amount: enhanced = x + amount*(x - blur(x)). "
                        "0 = off, 0.5 = gentle, 1.0 = aggressive (sr_v1 was too sharp).")
...
def main():
    ...
    # SR-anchor: propagate hyperparams to encode_batch state
    ...
    for epoch in range(start_epoch, start_epoch + args.num_epochs):
        ...
        for batch in pbar:
            ...
            enhanced_target_latents = cond["enhanced_target_latents"]   # SR-anchor's "real" for GAN-D
