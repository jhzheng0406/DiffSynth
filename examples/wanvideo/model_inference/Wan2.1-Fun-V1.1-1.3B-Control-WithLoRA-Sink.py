"""
Long video chain inference with the sink + recent LoRA.

For every chunk k in the chain:
  reference_image       = chunk_{k-1}'s last frame (or asset/6.png for chunk 0)
                          ← "recent" — same semantic as training's recent
  sink_reference_image  = INITIAL_REF (kept constant across all chunks)
                          ← "sink" — same semantic as training's video[0]

This matches the dual-ref training setup in train_chunk_aware.py +
WanVideoUnit_FunReference (sink at T=0, recent at T=1 in the concatenated
reference_latents).

Usage:
    python Wan2.1-Fun-V1.1-1.3B-Control-WithLoRA-Sink.py
"""
...
SEED             = 42
FPS              = 16
SAVE_PATH        = "samples/long_chunk_chain_V1.1_cartoon_sink_v2"
...
if not os.path.exists(LORA_PATH):
    raise FileNotFoundError(
        ...
        f"Train it first via lora/Wan2.1-Fun-V1.1-1.3B-Control_cartoon_sink.sh"
    )
...
# sink stays constant; recent updates each chunk
...
for k in range(NUM_CHUNKS):
    ...
    chunk_frames = pipe(
        ...
        reference_image=ref_for_chunk,            # recent
        sink_reference_image=sink_img,            # sink (always the same)
        height=HEIGHT,
        width=WIDTH,
    ...
    # Chain: this chunk's last frame becomes next chunk's recent
...
lora_tag = os.path.splitext(os.path.basename(LORA_PATH))[0]   # e.g. "step-200"
sink_tag = (
    f"_sink-{os.path.splitext(os.path.basename(INITIAL_REF))[0]}"
    if isinstance(INITIAL_REF, str) else ""
)
fname = f"V1.1-1.3B+LoRA-Sink-noref-Matchprompt-{lora_tag}{sink_tag}_a{LORA_ALPHA}_chunks{NUM_CHUNKS}x{CHUNK_FRAMES}_{HEIGHT}x{WIDTH}_seed{SEED}_fps{FPS}.mp4"
