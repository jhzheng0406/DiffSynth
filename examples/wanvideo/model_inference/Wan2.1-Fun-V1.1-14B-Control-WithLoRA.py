"""
Long video generation with a fine-tuned LoRA on Wan2.1-Fun-V1.1-14B-Control.

Same chunk-chain structure as Wan2.1-Fun-V1.1-14B-Control-Long.py — 10 chunks
of 49 frames, chained via last-frame reference — but with a LoRA loaded onto
pipe.dit. Use this to A/B compare a 14B LoRA-tuned chain vs the no-LoRA 14B
baseline.
...
Usage:
    python Wan2.1-Fun-V1.1-14B-Control-WithLoRA.py
"""
...
# Config
# ---------------------------------------------------------------------------
LORA_PATH        = "models/train/Wan2.1-Fun-V1.1-14B-Control_lora_cartoon_with_ref/step-4000.safetensors"
LORA_ALPHA       = 1.0
...
SEED             = 42
FPS              = 16
SAVE_PATH        = "samples/long_chunk_chain_V1.1_14B_cartoon_with_ref"
...
if not os.path.exists(LORA_PATH):
    raise FileNotFoundError(
        ...
        f"Train it first via lora/Wan2.1-Fun-V1.1-14B-Control_cartoon.sh"
    )
...
lora_tag = os.path.splitext(os.path.basename(LORA_PATH))[0]   # e.g. "step-1200"
ref_tag = (
    f"_ref-{os.path.splitext(os.path.basename(REFERENCE_IMAGE))[0]}"
    if isinstance(REFERENCE_IMAGE, str) else ""
)
fname = f"V1.1-14B+LoRA-{lora_tag}{ref_tag}_a{LORA_ALPHA}_chunks{NUM_CHUNKS}x{CHUNK_FRAMES}_{HEIGHT}x{WIDTH}_seed{SEED}_fps{FPS}.mp4"
