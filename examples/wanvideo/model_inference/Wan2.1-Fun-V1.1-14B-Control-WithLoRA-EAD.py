"""
Long video generation with the 14B cartoon+ref+EAD LoRA.
...
Usage:
    python Wan2.1-Fun-V1.1-14B-Control-WithLoRA-EAD.py
"""
...
# Config
# ---------------------------------------------------------------------------
LORA_PATH        = "models/train/Wan2.1-Fun-V1.1-14B-Control_lora_cartoon_ead/step-3980.safetensors"
LORA_ALPHA       = 1.0
...
SEED             = 42
FPS              = 16
SAVE_PATH        = "samples/long_chunk_chain_V1.1_14B_cartoon_ead"
...
if not os.path.exists(LORA_PATH):
    raise FileNotFoundError(
        ...
        f"Train it first via lora/Wan2.1-Fun-V1.1-14B-Control_cartoon_ead.sh"
    )
...
lora_tag = os.path.splitext(os.path.basename(LORA_PATH))[0]   # e.g. "step-3980"
ref_tag = (
    f"_ref-{os.path.splitext(os.path.basename(REFERENCE_IMAGE))[0]}"
    if isinstance(REFERENCE_IMAGE, str) else ""
)
fname = f"V1.1-14B+LoRA-EAD-{lora_tag}{ref_tag}_a{LORA_ALPHA}_chunks{NUM_CHUNKS}x{CHUNK_FRAMES}_{HEIGHT}x{WIDTH}_seed{SEED}_fps{FPS}.mp4"
