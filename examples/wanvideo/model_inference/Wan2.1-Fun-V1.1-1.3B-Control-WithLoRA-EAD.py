"""
Long video generation with the 1.3B cartoon+ref+EAD LoRA.

Same chunk-chain structure as Wan2.1-Fun-V1.1-1.3B-Control-Long.py — 10 chunks
of 49 frames, chained via last-frame reference — but with the EAD-trained
LoRA loaded onto pipe.dit.

This LoRA was trained with:
  - reference_image input (first frame of target video)
  - Helios EAD noise on reference_latents (corrupt_ratio=0.33, clean_prob=0.1)
so chain inference (where ref is the previous chunk's noisy last frame) is
in-distribution.

Usage:
    python Wan2.1-Fun-V1.1-1.3B-Control-WithLoRA-EAD.py
"""
...
# Config
# ---------------------------------------------------------------------------
LORA_PATH        = "models/train/Wan2.1-Fun-V1.1-1.3B-Control_lora_cartoon_ead_from_scratch/step-4975.safetensors"
LORA_ALPHA       = 1.0
...
SEED             = 42
FPS              = 16
SAVE_PATH        = "samples/long_chunk_chain_V1.1_cartoon_ead"
...
if not os.path.exists(LORA_PATH):
    raise FileNotFoundError(
        ...
        f"Train it first via lora/Wan2.1-Fun-V1.1-1.3B-Control_cartoon_ead.sh"
    )
...
lora_tag = os.path.splitext(os.path.basename(LORA_PATH))[0]   # e.g. "step-1990"
ref_tag = (
    f"_ref-{os.path.splitext(os.path.basename(REFERENCE_IMAGE))[0]}"
    if isinstance(REFERENCE_IMAGE, str) else ""
)
fname = f"V1.1-1.3B+LoRA-EAD-{lora_tag}{ref_tag}_a{LORA_ALPHA}_chunks{NUM_CHUNKS}x{CHUNK_FRAMES}_{HEIGHT}x{WIDTH}_seed{SEED}_fps{FPS}.mp4"
