"""
Long video generation — chunk-by-chunk with last-frame chaining (V1.1 14B).

Same chain structure as Wan2.1-Fun-V1.1-1.3B-Control-Long.py, but uses the
14B variant. Slower per step (~10x) but stronger generative quality. Use
this as the 14B baseline before any LoRA / EAD experiments at 14B.

Memory: bf16 + tiled VAE fits in 80GB (H100). For smaller GPUs, switch to
the offloading variant under examples/wanvideo/model_inference_low_vram/.

Usage:
    python Wan2.1-Fun-V1.1-14B-Control-Long.py
"""
...
SEED             = 42
FPS              = 16
SAVE_PATH        = "samples/long_chunk_chain_V1.1_14B_baseline"
...
fname = f"V1.1-14B{ref_tag}_chunks{NUM_CHUNKS}x{CHUNK_FRAMES}_{HEIGHT}x{WIDTH}_seed{SEED}_fps{FPS}.mp4"
