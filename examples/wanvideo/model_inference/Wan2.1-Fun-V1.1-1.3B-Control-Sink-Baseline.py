"""
Baseline for sink + chain-ref inference WITHOUT LoRA.

Same chain structure as Wan2.1-Fun-V1.1-1.3B-Control-WithLoRA-Sink.py:
  - sink_reference_image = INITIAL_REF (constant across all chunks)
  - reference_image      = chain ref (chunk_{k-1}'s last frame; sink for chunk 0)
But no LoRA is loaded — this is the unmodified V1.1 base + dual-ref pipeline.

Comparing this output vs the sink-LoRA output isolates "LoRA learned to use
sink+recent properly" from "the inference setup itself produces this result".

Usage:
    python Wan2.1-Fun-V1.1-1.3B-Control-Sink-Baseline.py
"""
...
# ---------------------------------------------------------------------------
# Config — keep IDENTICAL to WithLoRA-Sink for fair comparison
# ---------------------------------------------------------------------------
HEIGHT           = 832
WIDTH            = 480
CHUNK_FRAMES     = 49
...
SEED             = 42
FPS              = 16
SAVE_PATH        = "samples/long_chunk_chain_V1.1_sink_baseline"

# ---------------------------------------------------------------------------
# Load pipeline (Wan2.1-Fun-V1.1-1.3B-Control) — NO LoRA
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
all_frames = []
ref_for_chunk = sink_img    # chunk 0: recent = sink
...
fname = f"V1.1-1.3B-Sink-Matchprompt-Baseline{sink_tag}_chunks{NUM_CHUNKS}x{CHUNK_FRAMES}_{HEIGHT}x{WIDTH}_seed{SEED}_fps{FPS}.mp4"
