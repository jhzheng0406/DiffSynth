#!/bin/bash
# Inference for msgan_v5_midfreq = oneforcing + multi-scale PatchGAN (hybrid).
# 1-step + first_chunk_steps=4. Single GPU. Auto-picks latest checkpoint.
#
#   bash infer_wan1.3b_dmd_msgan_v5_midfreq.sh                  # latest step, 180s, GPU 0
#   STEP=200 DURATION=30 bash infer_wan1.3b_dmd_msgan_v5_midfreq.sh
...
DIR=models/train/wan1.3b_dmd_msgan_v5_midfreq
...
echo "[$(date +%H:%M:%S)] msgan_v5_midfreq step-$STEP  ${DURATION}s  on GPU $GPU"
echo "[student] $DIR"
...
CUDA_VISIBLE_DEVICES=$GPU bash $LONG --duration $DURATION --steps 1 \
    ...
    --student_dir $DIR \
    --student $STEP \
    2>&1 | tee "$LOGDIR/msgan_v5_midfreq_step${STEP}_${DURATION}s.log"
