#!/bin/bash
# Inference for gan_hf_v2 (DMD + One-Forcing GAN + VGG-feature HF aux).
# 1-step + first_chunk_steps=4. Single GPU.
#
#   bash infer_wan1.3b_dmd_gan_hf_v2.sh                  # step 200, 180s, GPU 0
#   STEP=150 DURATION=30 bash infer_wan1.3b_dmd_gan_hf_v2.sh
#   GPU=2 bash infer_wan1.3b_dmd_gan_hf_v2.sh
...
DIR=models/train/wan1.3b_dmd_gan_hf_v2
...
echo "[$(date +%H:%M:%S)] gan_hf_v2 step-$STEP  ${DURATION}s  on GPU $GPU"
echo "[student] $DIR"
...
CUDA_VISIBLE_DEVICES=$GPU bash $LONG --duration $DURATION --steps 1 \
    ...
    --student_dir $DIR \
    --student $STEP \
    2>&1 | tee "$LOGDIR/gan_hf_v2_step${STEP}_${DURATION}s.log"
