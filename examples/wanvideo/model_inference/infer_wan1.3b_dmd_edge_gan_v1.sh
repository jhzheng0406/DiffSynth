#!/bin/bash
# Inference for edge_gan_v1 = oneforcing + multi-scale PatchGAN (hybrid).
# 1-step + first_chunk_steps=4. Single GPU. Auto-picks latest checkpoint.
#
#   bash infer_wan1.3b_dmd_edge_gan_v1.sh                  # latest step, 180s, GPU 0
#   STEP=200 DURATION=30 bash infer_wan1.3b_dmd_edge_gan_v1.sh
...
DIR=models/train/wan1.3b_dmd_edge_gan_v1
...
echo "[$(date +%H:%M:%S)] edge_gan_v1 step-$STEP  ${DURATION}s  on GPU $GPU"
echo "[student] $DIR"
...
CUDA_VISIBLE_DEVICES=$GPU bash $LONG --duration $DURATION --steps 1 \
    ...
    --student_dir $DIR \
    --student $STEP \
    2>&1 | tee "$LOGDIR/edge_gan_v1_step${STEP}_${DURATION}s.log"
