"""
Long video generation — chunk-by-chunk with last-frame chaining.

Based on Wan2.1-Fun-1.3B-Control.py (V1.0). Each chunk is generated within
the model's trained window. For chunks k >= 1, reference_image is set to
the LAST FRAME of the previous chunk, anchoring appearance across the seam.

Note: V1.0 was not officially trained with reference_image input, but the
pipeline accepts it (CLIP weights are loaded). If ref has no effect, switch
to Wan2.1-Fun-V1.1-1.3B-Control-Long.py which uses the V1.1 variant.

Usage:
    python Wan2.1-Fun-1.3B-Control-Long.py
"""
...
REFERENCE_IMAGE  = None   # PIL.Image, path string, or None — used for chunk 0 only
...
SEED             = 42
FPS              = 16
SAVE_PATH        = "samples/long_chunk_chain_V1.0_baseline"

# ---------------------------------------------------------------------------
# Load pipeline (Wan2.1-Fun-1.3B-Control, V1.0)
# ---------------------------------------------------------------------------
...
fname = f"V1.0_chunks{NUM_CHUNKS}x{CHUNK_FRAMES}_{HEIGHT}x{WIDTH}_seed{SEED}_fps{FPS}.mp4"
